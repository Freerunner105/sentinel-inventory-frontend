...
const InventoryDashboard = () => {
  ...
  const fetchItemCodes = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5000/inventory/codes', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleReceiveSubmit = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/inventory/bulk', {
  ...
  const handleCodeSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/inventory/codes', {
  ...
  const handleConditionSelect = async (condition) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/inventory/remove', {
